...
function saveAndOpenErrorLog() {
  try {
    ...
    const logFileName = `uhao-api-crash-${timestamp}.log`;
...
function getBinaryPath() {
  ...
  if (isDev) {
    const binaryName = platform === 'win32' ? 'uhao-api.exe' : 'uhao-api';
...
function startServer() {
  return new Promise((resolve, reject) => {
    ...
    env.SQLITE_PATH = path.join(dataDir, 'uhao-api.db');
