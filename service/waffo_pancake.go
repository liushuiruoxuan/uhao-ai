...
// are exposed separately for self-service flows driven from uhao-api's own UI.
...
func WaffoPancakeBuyerIdentityFromUserID(userID int) string {
	return fmt.Sprintf("uhao-api-user-%d", userID)
}
...
const (
	defaultWaffoPancakeStoreName   = "uhao-api-store"
	defaultWaffoPancakeProductName = "uhao-api-charge-product"
)
...
//
// OnetimeProduct (not SubscriptionProduct) because uhao-api has no renewal-
// event handling; Pancake auto-renewing without uhao-api extending user
